...
original_author="deegok11"
original_name="flask_project"
original_urlname="flask_project"
original_description="Awesome flask_project created by deegok11" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_project "${name}"
...
then
    ...
    echo "then run 'flask_project' or 'python -m flask_project'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_project' or 'python -m flask_project' or 'poetry run flask_project'"
fi
